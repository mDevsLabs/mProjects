---
title: 将 mAI 连接到 iMessage
description: >-
  了解如何通过 mAI Desktop 本地 BlueBubbles 桥接，将 iMessage 连接到你的 mAI 智能体。
...
mAI 通过运行在已登录 Messages 的 Mac 上的 [BlueBubbles](https://bluebubbles.app/) 连接 iMessage。mAI Desktop 在同一台 Mac 上运行一个本地桥接：它在 `127.0.0.1` 接收 BlueBubbles 的 webhook，转发到 mAI 云端，并把智能体的回复经本地 BlueBubbles REST API 发回。BlueBubbles 无需暴露到公网。

```text
iMessage 用户 -> macOS Messages -> BlueBubbles -> mAI Desktop 桥接 -> mAI 云端
mAI 智能体 -> Device Gateway -> mAI Desktop 桥接 -> BlueBubbles -> iMessage 用户
```
...
- mAI Desktop 已登录，且已连接 Device Gateway（设置中显示网关已连接）

> **Private API 说明：** BlueBubbles 默认通过 AppleScript 发送基础文本和附件。打字指示等高级功能需要 BlueBubbles Private API（需关闭 SIP / 越狱的 Mac）。mAI 只依赖基础的文本与附件收发 —— 打字指示失败会被记录并忽略，不影响回复。
...
<Steps>
  ...
  在 BlueBubbles Server 中设置一个强密码。mAI Desktop 桥接会在本地用它调用 BlueBubbles REST API。
...
让**另一个人或第二个 Apple ID** 给托管在 BlueBubbles Mac 上的 Apple ID / 手机号发一条 iMessage。BlueBubbles 触发本地 `new-message` webhook，Desktop 桥接转发到 mAI，智能体在同一会话里回复。

> **为什么要用其他发送方？** 托管账号自己发的消息会被 `isFromMe` 防循环逻辑忽略（这样 Bot 不会回复自己发的消息）。用你自己的号码测试不会触发回复 —— 请用其他发送方。

## 功能说明

- **Markdown** —— iMessage 接收纯文本；mAI 在发送前会去除 Markdown 标记。
...
- **连接报 `DEVICE_NOT_FOUND`。** mAI Desktop 未通过 Device Gateway 可达。确认 Desktop 已打开、已登录、网关显示已连接，然后重新保存。
