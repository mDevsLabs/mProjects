---
title: Claude Code
description: 在 mAI 中委派 Anthropic Claude Code —— 通过桌面应用与 Claude Code CLI 对话，实时查看任务、待办、技能与工具调用，并跨轮次恢复会话。
tags:
  - mAI
  - Claude Code
  - 编程助理
...
Claude Code 是 Anthropic 推出的编程助理，能在终端中读取、修改、运行代码。在 mAI 中，你可以通过桌面应用委派 Claude Code —— 保留熟悉的对话界面，让 Claude Code 在本地完成实际工作，并完整访问你的项目。

发送一条提示，Claude Code 会读取文件、修改代码、运行命令，并把过程反馈给你。任务、待办、技能与工具调用会随着助理推进实时进入聊天；会话能跨轮次恢复，一项长任务可以分布在多条消息中持续推进。

## 什么是 mAI 中的 Claude Code？

它是 mAI 对话界面与本地 Claude Code CLI 之间的桥梁。mAI 在本地以子进程形式启动 `claude` 命令，把它的事件流接入聊天会话，并将 Claude Code 的输出 —— 增量消息、任务、待办、技能、子助理线程 —— 渲染为一等公民的聊天块。你用自然语言指挥助理，Claude Code 在本地用你的环境、凭据与项目上下文执行。

## 使用条件

- **mAI 桌面应用** —— Claude Code 助理只在桌面版可用，Web 端无法启动本地进程。
- **已安装 Claude Code CLI** —— `claude` 命令需要在你的 `PATH` 中可用。
- **已登录** —— 在 mAI 调用前，需在终端中先运行一次 `claude` 完成认证，需要 Anthropic 账号。
...
若 mAI 未能检测到 CLI，会弹出**安装 Claude Code CLI** 引导，并提供**打开系统工具**按钮 —— 安装完成后点击即可重新检测。

## 在 mAI 中添加 Claude Code

当 mAI 检测到本机已安装 Claude Code CLI，首页会出现一张标记为「编程助理」的**添加 Claude Code** 推荐卡片，点击即可一步创建 Claude Code 助理。
...
如果切换目录后，已保存的 Claude Code 会话无法恢复，mAI 会提示：**「工作目录已更改。之前的 Claude Code 会话只能在原始目录下恢复，已开启新的对话。」**

在工作目录内，Claude Code 以**完全访问**权限运行 —— 可对文件夹内任何文件进行读写。mAI 内部暂不支持切换权限模式。

## 聊天中会渲染什么

mAI 不会把 Claude Code 的工具调用渲染成原始 JSON，而是用专用区块呈现：
...
Claude Code 会话在同一话题中跨消息持续。mAI 会捕获底层 session ID 并在每次追问时复用，因此你可以随时回到长任务的任意进度点继续。
...
- **仅桌面端** —— Claude Code 助理只在 mAI 桌面应用中可用，Web 端无法启动 CLI。
- **每台机器一次登录** —— Claude Code 与全局 CLI 共享认证。终端里 `claude` 能用，mAI 里就能用。
- **绑定工作目录** —— 会话不会跨文件夹或机器跟随你。
- **仅支持完全访问** —— mAI 内部暂不支持切换权限模式。

## 使用技巧

- **先在终端中运行一次 `claude`** —— 登录在 CLI 层面完成，不在 mAI 里。
