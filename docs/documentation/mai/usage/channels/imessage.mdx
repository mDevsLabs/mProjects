---
title: Connect mAI to iMessage
description: >-
  Learn how to connect iMessage to your mAI agent through the local mAI Desktop BlueBubbles bridge.
...
# Connect mAI to iMessage

mAI connects to iMessage through [BlueBubbles](https://bluebubbles.app/) running on a Mac signed into Messages. The mAI Desktop app runs a local bridge on that same Mac: it receives BlueBubbles webhooks on `127.0.0.1`, forwards them to mAI Cloud, and relays the agent's replies back to the local BlueBubbles REST API. BlueBubbles never needs to be exposed to the public internet.

```text
iMessage user -> macOS Messages -> BlueBubbles -> mAI Desktop bridge -> mAI Cloud
mAI agent -> Device Gateway -> mAI Desktop bridge -> BlueBubbles -> iMessage user
```
...
- mAI Desktop signed in and connected to the Device Gateway (Settings shows the gateway as connected)

> **Private API note:** BlueBubbles sends basic text and attachments through AppleScript out of the box. Advanced features such as typing indicators require the BlueBubbles Private API, which needs SIP disabled / a jailbroken Mac. mAI only depends on basic text and attachment send/receive — typing-indicator failures are logged and ignored.
...
<Steps>
  ...
  In BlueBubbles Server, set a strong password. The mAI Desktop bridge uses it locally to call the BlueBubbles REST API.
...
<Steps>
  ...
  In mAI, go to **Settings → Advanced** and find the **Labs** section.
...
Have **another person or a second Apple ID** send an iMessage to the Apple ID / phone number signed into the BlueBubbles Mac. BlueBubbles fires a local `new-message` webhook, the Desktop bridge forwards it to mAI, and the agent replies in the same conversation.
...
- **Markdown** — iMessage receives plain text; mAI strips Markdown before sending.
...
- **Connect fails with `DEVICE_NOT_FOUND`.** mAI Desktop isn't reachable through the Device Gateway. Make sure Desktop is open, signed in, and the gateway shows as connected, then save again.
