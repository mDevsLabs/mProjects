...
Cloud Sandbox powers the built-in code execution, shell command, file operation, and file export tools. By default, mAI uses the Market sandbox. Self-hosted deployments can switch the same tool surface to an Onlyboxes-compatible sandbox provider.
...
- Type: Optional
- Description: Market Trusted Client ID used by the mAI server to call Market capabilities on behalf of the current user. It must be registered by the target Market service.
- Default: -
...
- Type: Optional
- Description: Lifetime of each Onlyboxes JIT token minted by mAI.
- Default: `1800`
- Example: `900`
...
By default, mAI uses the official Market sandbox and does not require extra sandbox configuration:
...
If that Market service requires the mAI server to call sandbox, credential, or skill capabilities on behalf of the current user, also configure Trusted Client credentials:
...
The configured Onlyboxes worker should expose `terminalExec` and `terminalResource`. mAI uses `terminalExec` as the compatibility layer for shell commands, code execution, and file operations, and uses `terminalResource` for file export through a pre-signed upload URL.
...
<Callout type={'info'}>
  File export still writes the exported artifact to the configured mAI S3 storage. Configure the
