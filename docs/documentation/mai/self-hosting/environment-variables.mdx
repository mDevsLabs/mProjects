---
title: mAI Environment Variables - Customizing Guide
description: >-
  Learn how to customize mAI configuration using environment variables for
...
mAI provides some additional configuration options when deployed, which can be customized using environment variables.
...
If you need to override `NEXT_PUBLIC` environment variables, you can build a custom Docker image using GitHub Actions without forking the entire mAI repository. Here's a guide on how to do this:
...
6. To build your custom image, go to the "Actions" tab in your GitHub repository and manually trigger the "Build Custom mAI Image" workflow.

This approach allows you to create a custom build with your desired `NEXT_PUBLIC` variables without maintaining a full fork of the mAI repository. You can trigger a new build whenever you need to update your custom image.
