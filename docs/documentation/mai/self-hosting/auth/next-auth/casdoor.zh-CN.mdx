---
title: 在 mAI 中配置 Casdoor 身份验证服务
description: 学习如何在 mAI 中配置 Casdoor 身份验证服务，包括部署、创建、设置权限和环境变量。
tags:
  - Casdoor 身份验证
  - 环境变量配置
  - 单点登录
...
<Callout type={'tip'}>
  若你想要私有部署 Casdoor，我们建议你将之与 mAI 一同使用 Docker Compose 部署，此时
  mAI 可以与之共用同一个 Postgres 实例。
</Callout>
...
<Steps>
  ...
  前往 `身份认证` -> `应用`，创建一个 `mAI` 应用或直接修改内置的 `built-in` 应用，其他字段可以自行探索，但你至少需要配置以下字段：

  - 名称、显示名称：`mAI`
  - 重定向 URLs：
    - 本地开发环境：`http://localhost:3210/api/auth/callback/casdoor`
    - 局域网 IP 部署：`http://mAI_IP:3210/api/auth/callback/casdoor`
  ...
  配置 Casdoor 的 Webhook 以便在用户信息更新时同步到 mAI 。
  ...
  将获取到的 `客户端 ID` 和 `客户端`，设为 mAI 环境变量中的 `AUTH_CASDOOR_ID` 和 `AUTH_CASDOOR_SECRET`。

  配置 mAI 环境变量中 `AUTH_CASDOOR_ISSUER` 为：
...
<Callout type={'info'}>部署成功后，用户将可以通过 Casdoor 身份认证并使用 mAI。</Callout>
