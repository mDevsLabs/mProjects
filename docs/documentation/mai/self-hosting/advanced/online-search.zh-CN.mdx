...
description: 了解如何为 mAI 配置 SearXNG 联网搜索功能，使 AI 能够获取最新的网络信息。
tags:
  - 联网搜索
  - SearXNG
  - 网络信息
...
mAI 支持为 AI 配置**联网搜索功能**，使其能够实时获取互联网信息，从而提供更准确、最新的回答。联网搜索支持多个搜索引擎提供商，包括 [SearXNG](https://github.com/searxng/searxng)、[Search1API](https://www.search1api.com)、[Google](https://programmablesearchengine.google.com)、[Brave](https://brave.com/search/api) 等。
...
通过设置搜索服务环境变量 `SEARCH_PROVIDERS` 和对应的 API Key，mAI 将在多个搜索源中查询并返回结果。你还可以搭配配置爬虫服务环境变量 `CRAWLER_IMPLS`（如 `browserless`、`firecrawl`、`tavily` 等）以提取网页内容，实现搜索 + 阅读的增强能力。
...
您可以在 [SearXNG 实例列表](https://searx.space/) 中找到公开可用的 SearXNG 实例。选择一个响应速度快、可靠性高的实例，然后将其 URL 配置到 mAI 中。
...
- [mAI 联网搜索 RFC 讨论](https://github.com/lobehub/lobehub/discussions/6447)
