...
description: >-
  Learn how to configure Redis cache service to optimize mAI performance and
  session management.
tags:
  - Redis
  - Cache
...
mAI uses Redis as a high-performance cache and session storage service to optimize system performance and manage user authentication state.

<Callout type={'info'}>
  mAI uses the standard Redis protocol (via ioredis library), supporting any Redis
...
Redis is used in mAI for the following scenarios:
...
<Steps>
  ...
  The prefix for Redis keys, used to isolate mAI data in a shared Redis instance.
...
<Callout type={'warning'}>
  Redis is an optional service. If `REDIS_URL` is not configured, mAI will still function
