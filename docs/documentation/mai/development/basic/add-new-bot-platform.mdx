...
description: >-
  Learn how to add a new bot platform (e.g., Slack, WhatsApp) to mAI's
...
This guide walks through the steps to add a new bot platform to mAI's channel system. The platform architecture is modular — each platform is a self-contained directory under `apps/server/src/services/bot/platforms/`.
...
Follow the structure of existing docs (e.g., `discord.mdx`): Prerequisites → Create App → Configure in mAI → Configure Webhooks → Test Connection → Configuration Reference → Troubleshooting.
