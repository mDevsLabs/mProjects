---
title: Connect mAI to Slack
description: >-
  Learn how to create a Slack app and connect it to your mAI agent as a
...
# Connect mAI to Slack

By connecting a Slack channel to your mAI agent, users can interact with the AI assistant directly through Slack channels and direct messages.
...
mAI supports two connection modes for Slack:
...
<Steps>
  ...
  display_information:
    name: mAI Assistant
    description: AI assistant powered by mAI
  features:
    app_home:
...
### Step 3: Configure Slack in mAI

<Steps>
  ### Open Channel Settings

  In mAI, navigate to your agent's settings → **Channels** tab → click **Slack**.
...
Click **Test Connection** in mAI, then go to Slack, invite the bot to a channel, and mention it with `@mAI Assistant` to confirm it responds.
...
<Steps>
  ...
  Enter **Application ID**, **Bot Token**, and **Signing Secret** in mAI's Slack channel settings. Set **Connection Mode** to **Webhook** in Advanced Settings. Save and copy the displayed **Webhook URL**.
...
To get it: in Slack, click your avatar → **Profile**, then click the `⋮` overflow menu and choose **Copy member ID**. Member IDs start with `U`. Paste it into **Your Platform User ID** in mAI's Advanced Settings.
...
To get it: open Slack in the browser; the URL contains the team ID (`https://app.slack.com/client/T01ABCDEF/...`) — copy the part starting with `T`. Paste it into **Default Server** in mAI's Advanced Settings.
...
- **Bot ignores DMs even though Slack is configured correctly:** Open **Advanced Settings** in mAI and check **DM Policy**. If it is `Disabled`, switch to `Open` or `Allowlist`. If it is `Allowlist`, confirm the sender's Slack member ID is listed in **Allowed User IDs**.
...
- **`missing_scope` error (e.g., `needed: 'users:read'`) in logs:** Your app is missing a required OAuth scope, or you added a scope but never reinstalled the app. The `users:read` scope is required to resolve `@username` mentions in messages. Go to **OAuth & Permissions** → **Bot Token Scopes**, confirm all scopes listed in the manifest are present, then click **Reinstall to Workspace** at the top of the page and reauthorize. Update the **Bot User OAuth Token** in mAI if it changes.
