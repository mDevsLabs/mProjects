---
title: 在 mAI 中配置 Keycloak 身份验证服务
description: 学习如何在 mAI 中配置 Keycloak 身份验证服务，包括部署、创建、设置权限和环境变量。
tags:
  - Keycloak 身份验证
  - 环境变量配置
  - 单点登录
...
<Callout type={'warning'}>
  NextAuth（Auth.js）已从 mAI 中完全移除。下文出现的 `NEXT_AUTH_SSO_PROVIDERS` 和 `AUTH_URL` 等变量现在会导致部署在构建 / 启动阶段失败。本页仅作历史参考保留，请改用 [Better Auth 提供商指南](/zh/docs/self-hosting/auth) 和 [NextAuth 迁移至 Better Auth 指南](/zh/docs/self-hosting/migration/v2/auth/nextauth-to-betterauth)。
</Callout>

[Keycloak](https://www.keycloak.org/) 是一个开源的身份和访问管理解决方案，提供单点登录、身份代理和社交登录等功能，适用于现代应用和服务。

<Callout type={'tip'}>
  若你想要私有部署 Keycloak，我们建议你将之与 mAI 一同使用 Docker Compose
  部署，这样可以更方便地管理服务。
</Callout>
...
- 你的 mAI IP / 端口为 `http://mAI_IP:3210`。
- 你私有部署 Keycloak，其域名为 `http://KEYCLOAK_IP:8080`。

若你使用公网部署，下文假设：

- 你的 mAI 域名为 `https://lobe.example.com`。
...
<Steps>
  ...
  2. 创建客户端（Client）
     ...
     - 在 "Login settings" 页面：
       - Valid redirect URIs:
         ...
         - 局域网 IP 部署：`http://mAI_IP:3210/api/auth/callback/keycloak`
         - 公网环境：`https://lobe.example.com/api/auth/callback/keycloak`
       - Web origins: 添加你的 mAI 域名或 IP
       - 点击 "Save"
  ...
  将获取到的客户端 ID 和客户端密钥，设为 mAI 环境变量中的 `AUTH_KEYCLOAK_ID` 和 `AUTH_KEYCLOAK_SECRET`。

  配置 mAI 环境变量中 `AUTH_KEYCLOAK_ISSUER` 为：

  - `http://localhost:8080/realms/mAI`，若你是本地开发环境
  - `http://KEYCLOAK_IP:8080/realms/mAI`，若你是局域网私有部署的 Keycloak
  - `https://lobe-auth-api.example.com/realms/mAI`，若你是公网环境部署的 Keycloak

  在部署 mAI 时，你需要配置以下环境变量：
...
<Callout type={'info'}>部署成功后，用户将可以通过 Keycloak 身份认证并使用 mAI。</Callout>
