---
title: 在 mAI 中配置 Cloudflare Zero Trust 身份验证服务
description: >-
  学习如何在 mAI 中配置 Cloudflare Zero Trust 身份验证服务，包括创建提供程序、配置环境变量和部署
  mAI。详细步骤和必要环境变量设置。
tags:
...
<Steps>
  ...
  我们现在默认您已经了解了如何使用 Cloudflare Zero Trust 平台且假设您的 mAI 实例部署在 `https://chat.example.com` 中。
  ...
  在 `Application` 文本框内填入应用名称，如：`mAI SSO`，然后点击 `Select OIDC` 后点击 `Add applicaiton`

  ![添加 mAI SSO 应用](/blog/assets0ceb7e446f9a850df283093563ba7803.webp)

  至此您已成功在 Clouflare Zero Trust 中创建了一个名为 `mAI SSO` 的 SaaS 应用。
  ...
  最后我们将页面往下滚动，您将需要记录以下三个值 `Client secret`, `Client ID` 及 `Issuer` 以备后续部署 mAI 环境变量使用。
...
<Callout type={'info'}>
  部署成功后，用户将可以使用 Cloudflare Zero Trust 中配置的用户通过身份认证并使用 mAI。
</Callout>
