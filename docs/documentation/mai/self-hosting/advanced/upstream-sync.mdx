---
title: mAI Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for mAI Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest mAI image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running mAI Container

  Assuming the mAI container is named `lobehub`, use the following commands to stop and remove the currently running mAI container:
  ...
  ### Pull the Latest mAI Image

  Use the following command to pull the latest Docker image for mAI:
  ...
  Redeploy the mAI container using the newly pulled image:
...
<Callout type={'tip'}>
  **If I redeploy, will I lose my local chat records?**
  ...
  No need to worry, you won't. All of mAI's chat records are stored in your local browser. Therefore, when redeploying mAI using Docker, your chat records will not be lost.
</Callout>
