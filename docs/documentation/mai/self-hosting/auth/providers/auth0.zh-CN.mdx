---
title: 在 mAI 中配置 Auth0 身份验证
description: 学习如何在 mAI 中配置 Auth0 SSO，包括创建应用、添加用户和设置环境变量。
tags:
  - Auth0
  - 身份验证
  - mAI
...
<Callout type={'info'}>
  部署成功后，用户将可以通过 Auth0 身份认证并使用 mAI。
</Callout>
...
<Callout type={'warning'}>
  创建 GitHub 连接时，务必在 **Attributes** 部分勾选 **Email Address** 权限。mAI 需要用户邮箱进行身份认证。
