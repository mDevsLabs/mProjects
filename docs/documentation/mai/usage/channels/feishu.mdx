---
title: Connect mAI to Feishu (飞书)
description: >-
  Learn how to create a Feishu custom app and connect it to your mAI agent
...
# Connect mAI to Feishu (飞书)

By connecting a Feishu channel to your mAI agent, team members can interact with the AI assistant directly on Feishu through private chats and group conversations.
...
mAI supports two connection modes for Feishu bots:
...
<Steps>
  ...
  Click **Create Enterprise App**. Fill in the app name (e.g., "mAI 助手"), description, and icon, then submit the form.
...
## Step 3: Configure Feishu in mAI

<Steps>
  ### Open Channel Settings

  In mAI, navigate to your agent's settings, then select the **Channels** tab. Click **飞书** (Feishu) from the platform list.
...
Back in mAI's channel settings, click **Test Connection** to verify the credentials. Then find your bot on Feishu by searching its name and send it a message to confirm it responds.
...
To get it: DM the bot once and inspect the inbound event payload — the `open_id` field on the sender is yours. The Feishu Developer Portal also exposes a **User ID** lookup that maps mobile/email to `open_id`. Paste it into **Your Platform User ID** in mAI's Advanced Settings.
