---
title: 数据存储取数模块
description: 了解 mAI 数据存储取数模块及其选择器的使用和优势。
tags:
  - mAI
  - 数据存储
  - 选择器
...
selectors 是 mAI 数据流研发框架下的取数模块，它的作用是从 store 中以特定特务逻辑取出数据，供组件消费使用。
...
4. **类型安全**：由于 mAI 使用 TypeScript，每个选择器都有明确的输入和输出类型定义。这为开发者提供了自动完成和编译时检查的优势，减少了运行时错误。
...
通过这样的设计，mAI 的开发者可以更专注于构建用户界面和业务逻辑，而不必担心数据的获取和处理细节。这种模式也为未来可能的状态结构变更提供了更好的适应性和扩展性。
