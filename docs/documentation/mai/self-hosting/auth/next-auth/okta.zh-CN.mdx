---
title: 在 mAI 中配置 Okta 身份验证服务 - 详细步骤和环境变量设置
description: 学习如何在 mAI 中为您的组织配置 Okta 身份验证服务，包括创建应用程序、添加用户和配置环境变量等。
tags:
  - Okta
  - 身份验证
  - 单点登录
...
<Steps>
  ...
  | 设置名称                   | 描述                                          | 示例信息                                          |
  | ---------------------- | ------------------------------------------- | --------------------------------------------- |
  | App Integration Name   | 您的用户将看到的应用程序名称                              | mAI Instance                              |
  ...
  点击顶部导航栏中的「Assignments」进入用户管理界面，您可以在此创建或分配组织中的用户来登录 mAI。

  ### 配置环境变量

  在部署 mAI 时，您需要配置以下环境变量：
...
<Callout>
  部署成功后，用户将能够使用在 Okta 中配置的用户进行身份验证并使用 mAI。
</Callout>
