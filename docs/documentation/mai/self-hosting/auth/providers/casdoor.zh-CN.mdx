---
title: 在 mAI 中配置 Casdoor 身份验证
description: 学习如何在 mAI 中配置 Casdoor SSO，包括创建应用和设置环境变量。
tags:
  - Casdoor
  - 身份验证
  - mAI
...
<Steps>
  ...
  配置 Casdoor 的 [Webhook](https://www.casdoor.org/docs/webhooks/overview#setting-up-a-webhook) 以便在用户信息更新时同步到 mAI。
...
如果你使用 Docker Compose 部署 mAI，可以参考以下配置来集成 Casdoor 作为鉴权服务。
...
1. mAI 需要与 Casdoor 通讯，因此你需要配置 Casdoor 的 Issuer：

```env
AUTH_CASDOOR_ISSUER=https://auth.example.com
```

该配置会影响 mAI 的登录鉴权服务，你需要确保 Casdoor 服务的地址正确。

2. 在 Casdoor 中允许回调地址为 mAI 的地址：
...
- 请检查你的鉴权服务是否正常运行，以及 mAI 所在的网络是否能够访问到鉴权服务。

- 一个直接的排查方式，你可以在 mAI 容器的终端中，使用 `curl` 命令访问你的鉴权服务 `https://auth.example.com/.well-known/openid-configuration`，如果返回了 JSON 格式的数据，则说明你的鉴权服务正常运行。
