...
mAI 支持多模态的 AI 会话，包括将图片、文件等非结构化数据上传给大模型的功能。为了优化存储和性能，我们使用 S3 兼容的文件存储服务来存储图片文件和支持文件上传 / 知识库功能。
...
当设置为 `1` 时，mAI 会将图片转换为 base64 编码后上传到 LLM 模型中，当遇到如下错误时请考虑配置该环境变量为 1
...
当设置为 `1` 时，mAI 会将视频转换为 base64 编码后上传到 LLM 模型中。
