---
title: Configuring Casdoor Authentication for mAI
description: >-
  Learn how to configure Casdoor SSO for mAI, including creating an
...
<Steps>
  ...
  | `AUTH_SSO_PROVIDERS`     | Required | SSO provider for mAI. Use `casdoor` for Casdoor                           |
  ...
  Configure Casdoor [Webhook](https://www.casdoor.org/docs/webhooks/overview#setting-up-a-webhook) to sync user data updates to mAI.
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate with Casdoor and use mAI.
</Callout>

## Docker Compose Deployment with Casdoor

If you're deploying mAI using Docker Compose, refer to the following configuration to integrate Casdoor as an authentication service.
...
1. mAI needs to communicate with Casdoor, so you need to configure Casdoor's Issuer:

```env
AUTH_CASDOOR_ISSUER=https://auth.example.com
```

This configuration affects mAI's login authentication service. Ensure the Casdoor service URL is correct.

2. Allow callback URL in Casdoor to point to mAI:
...
Cause: mAI cannot access the authentication service.

Solution:

- Check if your authentication service is running properly and if mAI's network can reach it.

- Direct troubleshooting: Use `curl` in the mAI container terminal to access `https://auth.example.com/.well-known/openid-configuration`. If JSON data is returned, your authentication service is working correctly.
