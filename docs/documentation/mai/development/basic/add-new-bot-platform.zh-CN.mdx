---
title: 添加新的 Bot 平台
description: 了解如何向 mAI 的渠道系统添加新的 Bot 平台（如 Slack、WhatsApp），包括 Schema 定义、客户端实现和平台注册。
tags:
  - Bot 平台
  - 消息渠道
  - 集成
...
本指南介绍如何向 mAI 的渠道系统添加新的 Bot 平台。平台架构是模块化的 —— 每个平台是 `apps/server/src/services/bot/platforms/` 下的一个独立目录。
...
参考现有文档的结构（如 `discord.mdx`）：前置条件 → 创建应用 → 在 mAI 中配置 → 配置 Webhook → 测试连接 → 配置参考 → 故障排除。
