...
description: >-
  Connect your mAI agents to external messaging platforms like Discord,
...
Channels allow you to connect your mAI agents to external messaging platforms. Once connected, users can interact with your AI assistant directly in the chat apps they already use — no need to visit mAI.
...
| [iMessage](/docs/usage/channels/imessage)  | Connect to iMessage through the local mAI Desktop BlueBubbles bridge (Labs)               |
...
Each channel integration works by linking a bot account on the target platform to a mAI agent. When a user sends a message to the bot, mAI processes it through the agent and sends the response back to the same conversation.
...
- **Multiple channels simultaneously** — A single agent can be connected to Discord, Slack, Telegram, LINE, QQ, WeChat, Feishu, and Lark at the same time. mAI routes messages to the correct agent automatically.
