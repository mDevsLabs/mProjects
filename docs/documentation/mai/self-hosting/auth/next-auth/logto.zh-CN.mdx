---
title: 在 mAI 中配置 Logto 身份验证服务
description: 学习如何在 mAI 中配置 Logto 身份验证服务，包括部署、创建、设置权限和环境变量。
tags:
  - Logto 身份验证
  - 环境变量配置
  - 单点登录
...
<Callout type={'tip'}>
  若你想要私有部署 Logto，我们建议你将之与 mAI 一同使用 Docker Compose 部署，此时
  mAI 可以与之共用同一个 Postgres 实例。
</Callout>

## Logto 配置流程

下文假设你的 mAI 域名为 `https://lobe.example.com`。
...
<Steps>
  ...
  配置 Logto 的 Webhook，以便在用户信息更新时 mAI 可以接收到通知。

  前往 `Webhooks` ，创建一个 Webhook，填写以下字段：

  - 端点 URL： `https://lobe.example.com/api/webhooks/logto`
  - 事件：
    - `User.Data.Updated`: 允许 mAI 同步 Logto 中用户资料信息的更新。
    - `User.SuspensionStatus.Updated`: 允许 mAI 将被暂停的用户移除登录会话，仅在数据库会话策略为 `database` 时可用。
  ...
  将获取到的 `Client ID` 和 `Client Secret`，设为 mAI 环境变量中的 `AUTH_LOGTO_ID` 和 `AUTH_LOGTO_SECRET`。

  配置 mAI 环境变量中 `AUTH_LOGTO_ISSUER` 为：
...
<Callout type={'info'}>部署成功后，用户将可以通过 Logto 身份认证并使用 mAI。</Callout>
