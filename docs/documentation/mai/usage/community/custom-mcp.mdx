...
title: Custom MCP
description: >-
  Connect your own MCP servers to mAI — add private APIs, internal tools,
...
The MCP Marketplace offers thousands of ready-to-use integrations, but sometimes you need to connect a private API, an internal company tool, or a custom MCP server you've built yourself. **Custom MCP** lets you add any MCP-compatible server directly to mAI without going through the marketplace.
...
<Steps>
  ...
  mAI automatically fills in the identifier, connection type, URL or command, and any other detected parameters. Review the auto-filled fields, adjust if needed, then proceed to test the connection.
</Steps>
...
Use this for local MCP servers that run as a process on your machine — command-line tools, scripts, or locally installed packages. **Available on the mAI desktop app only; not supported on the web.**
...
Before saving, click **Test connection** to verify that mAI can reach your MCP server and fetch its tool list. If the test succeeds, you'll see the available tools listed in the preview panel on the right.
