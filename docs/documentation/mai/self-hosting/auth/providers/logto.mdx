---
title: Configuring Logto Authentication for mAI
description: >-
  Learn how to configure Logto SSO for mAI, including creating an
...
<Steps>
  ...
  4. Fill in the application name: `mAI`
  ...
  | `AUTH_SSO_PROVIDERS`        | Required | SSO provider for mAI. Use `logto` for Logto                               |
  ...
  Configure Logto [Webhook](https://docs.logto.io/developers/webhooks/configure-webhooks) to sync user data updates to mAI.
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate with Logto and use mAI.
</Callout>
