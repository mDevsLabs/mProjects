...
云端沙箱用于内置的代码执行、Shell 命令、文件操作和文件导出工具。默认情况下，mAI 使用 Market 沙箱；自托管部署可以把同一套工具能力切换到兼容 Onlyboxes 的沙箱提供方。
...
- 类型：可选
- 描述：Market Trusted Client 的客户端 ID，用于让 mAI 服务端代表当前用户调用 Market 能力。需要由对应 Market 服务登记。
- 默认值：-
- 示例：`lobechat-com`
...
- 类型：可选
- 描述：mAI 签发的每个 Onlyboxes JIT Token 的有效期。
- 默认值：`1800`
- 示例：`900`
...
默认情况下，mAI 使用官方 Market 沙箱，不需要额外配置：
...
如果该 Market 服务要求 mAI 服务端代表当前用户调用沙箱、凭据或技能能力，还需要配置 Trusted Client：
...
配置的 Onlyboxes worker 需要暴露 `terminalExec` 和 `terminalResource`。mAI 使用 `terminalExec` 作为 Shell 命令、代码执行和文件操作的兼容层，并使用 `terminalResource` 通过预签名上传 URL 导出文件。
...
<Callout type={'info'}>
  文件导出仍会把沙箱内产物写入 mAI 配置的 S3 存储。如果用户需要下载沙箱生成的文件，请同时配置 S3
  相关环境变量。
</Callout>
