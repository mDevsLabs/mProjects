...
description: NextAuth 和 Clerk 这两种旧版身份验证方法的历史参考文档，二者均已从 mAI 移除，请改用 Better Auth。
tags:
  - 身份验证服务
  - Next Auth
  - Clerk
...
<Callout type={'warning'}>
  **旧版提示**：NextAuth 和 Clerk 已从 mAI 中完全移除。设置已移除的 NextAuth/Clerk 变量（`NEXT_AUTH*` / `NEXTAUTH*`、Clerk 密钥、`AUTH_URL`、`NEXT_PUBLIC_AUTH_URL`、`ACCESS_CODE` 等）会在构建 / 启动阶段失败。**`AUTH_SECRET` 并未废弃**，[Better Auth](/zh/docs/self-hosting/auth) 仍需要它。本页仅供历史迁移参考。
</Callout>

以下认证方式（NextAuth、Clerk）已从 mAI 移除。已废弃的 NextAuth/Clerk 变量会导致部署失败；`AUTH_SECRET` 仍由 Better Auth 使用。本页面仅供历史迁移参考。

## Clerk

Clerk 是一个全面的身份验证解决方案，它提供了简单而强大的 API 和服务来处理用户认证和会话管理。

mAI 与 Clerk 做了深度集成，能够为用户提供安全、便捷的登录和注册体验。

在 mAI 的环境变量中设置 `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` 和 `CLERK_SECRET_KEY`，即可开启和使用 Clerk。
...
在使用 NextAuth 之前，请先在 mAI 的环境变量中设置以下变量：

| 环境变量                             | 类型      | 描述                                                                                                                       |
| -------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------ |
| `NEXT_PUBLIC_ENABLE_NEXT_AUTH`   | *(已废弃)* | 用于启用 NextAuth 服务。NextAuth 已从 mAI 中移除，设置该变量会导致部署在构建 / 启动阶段失败。                                                         |
...
允许 mAI 在身份提供商中用户信息更新时接收通知。支持的提供商包括 Casdoor 和 Logto。请参考具体提供商文档进行配置。
