---
title: 在 Zeabur 上部署 mAI
description: 按照指南准备 OpenAI API Key 并点击按钮进行部署。在部署完成后，即可开始使用 mAI 并选择是否绑定自定义域名。
tags:
  - Zeabur
  - mAI
  - OpenAI API Key
...
这里是在 Zeabur 上部署 mAI 服务器数据库版的流程：

<Steps>
  ### 前往 Zeabur 上的模板页面

  前往 [Zeabur 上的 mAI 数据库模板页面](https://zeabur.com/templates/RRSPSD) 并点击 "Deploy" 按钮。
  ...
  - mAI Domain: 一个免费的 `.zeabur.app` 后缀的域名。
  ...
  在你填写完所有必要的环境变量后，选择一个你想要部署 mAI 数据库的区域并点击 “部署” 按钮。
  ...
  使用你刚绑定的域名来访问你的 Logto 控制台，创建一个新项目以获得对应的客户端 ID 与密钥，将它们填入你的 mAI 服务的变量中。关于如何填入变量，可以参照 [Zeabur 的官方文档](https://zeabur.com/docs/deploy/variables)。

  Logto 的详细配置可以参考[这篇文档](/zh/docs/self-hosting/auth/providers/logto)。

  ### 访问你的 mAI

  按下 `mAI-Database` 你会看到你刚刚创建的公共域名，点击它以访问你的 mAI 数据库。
