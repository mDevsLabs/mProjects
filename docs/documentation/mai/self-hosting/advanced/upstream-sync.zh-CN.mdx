---
title: mAI 启动自动更新 - Vercel / Zeabur 部署
description: 按照指南重新部署项目以解决 mAI 在 Vercel 中默认创建新项目导致的“有可用更新”提示问题，并启用自动更新。
tags:
  - Vercel
  - Zeabur
  - 自动更新
...
Docker 部署版本的升级非常简单，只需要重新部署 mAI 的最新镜像即可。 以下是执行这些步骤所需的指令：

<Steps>
  ### 停止并删除当前运行的 mAI 容器

  假设 mAI 容器的名称是 `lobehub`，使用以下指令停止并删除当前运行的 mAI 容器：
...
<Callout type={'tip'}>
  **重新部署的话，我本地的聊天记录会丢失吗？**
  ...
  放心，不会的。mAI 的聊天记录全部都存储在你的本地浏览器中。因此使用 Docker 重新部署 mAI 时，你的聊天记录并不会丢失。
</Callout>
