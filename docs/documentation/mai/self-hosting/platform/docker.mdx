---
title: Deploying mAI Database with Docker
description: >-
  Learn how to deploy the mAI server database version using Docker on Linux
  and local machines.
tags:
  - mAI
  - Docker
...
Here is the process for deploying the mAI server database version on a Linux server:
...
<Steps>
  ...
  <Callout type="info">
    ...
    components for mAI to implement RAG and knowledge base search.

    It should be noted that the new version of ParadeDB requires explicitly enabling the pg\_search extension in order to be used by mAI. Here we explicitly enable pg\_search.
  </Callout>
...
The data version of mAI also supports direct use on a local Mac/Windows machine.
