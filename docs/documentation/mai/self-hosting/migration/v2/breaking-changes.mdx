---
title: mAI 2.0 Breaking Changes
description: >-
  A comprehensive guide to breaking changes in mAI 2.0 and how to migrate
  from version 1.x.
tags:
  - mAI 2.0
  - Migration
...
# mAI 2.0 Breaking Changes

This document outlines the breaking changes introduced in mAI 2.0 and provides migration guidance for users upgrading from version 1.x.

## Removed Environment Variables

The following environment variables have been removed in mAI 2.0:
...
mAI 2.0 only supports Better Auth authentication system. The following environment variables are now required:
...
mAI 2.0 only supports Better Auth authentication system. NextAuth and Clerk are no longer supported.
...
mAI 2.0 only supports Server DB mode. Client DB (PGlite) is no longer supported. If you were using `NEXT_PUBLIC_SERVICE_MODE=client`, you need to migrate to Server DB deployment.

For deployment guides, see [Getting Started](/docs/self-hosting/start).

## PostgreSQL Version Requirements

mAI 2.0 recommends using **PostgreSQL 17** or higher.

This is because mAI 2.0 uses the [pg\_search](https://github.com/paradedb/paradedb/tree/main/pg_search) extension for full-text search capabilities. If you use Serverless Postgres services like Neon, the pg\_search extension is only available on PostgreSQL 17.
