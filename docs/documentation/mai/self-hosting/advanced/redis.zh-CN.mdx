---
title: 配置 Redis 缓存服务
description: 了解如何配置 Redis 缓存服务以优化 mAI 的性能和会话管理。
tags:
  - Redis
  - 缓存
  - 会话存储
...
mAI 使用 Redis 作为高性能缓存和会话存储服务，用于优化系统性能和管理用户认证状态。

<Callout type={'info'}>
  mAI 使用标准 Redis 协议（通过 ioredis 库），支持任何兼容 Redis 协议的服务，包括 Redis
...
<Steps>
  ...
  Redis 键的前缀，用于在共享 Redis 实例中隔离 mAI 的数据。
...
<Callout type={'warning'}>
  Redis 是可选服务。如果不配置 `REDIS_URL`，mAI 仍然可以正常运行，但会失去上述缓存和会话管理的优化功能。
</Callout>
