...
mAI supports multimodal AI sessions, including the ability to upload unstructured data such as images and files to large language models. To optimize storage and performance, we use S3-compatible file storage services to store image files and support file upload/knowledge base functionality.
...
When set to `1`, mAI will convert images to base64 encoding before uploading them to the LLM model. When encountering the following error, please consider configuring this environment variable to `1`:
...
When set to `1`, mAI will convert videos to base64 encoding before uploading them to the LLM model.
