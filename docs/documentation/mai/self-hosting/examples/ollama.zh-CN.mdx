---
title: 在 mAI 中集成 Ollama
description: 学习如何在 mAI 中配置与部署 Ollama 语言模型框架，提升本地大模型使用体验。
tags:
  - Ollama
  - mAI
  - 语言模型
...
Ollama 是一款强大的本地运行大型语言模型（LLM）的框架，支持多种语言模型，包括 Llama 2, Mistral 等。现在，mAI 已经支持与 Ollama 的集成，这意味着你可以在 mAI 中轻松使用 Ollama 提供的语言模型来增强你的应用。

本文档将指导你如何配置与部署 mAI 来使用 Ollama：
...
假设你已经在本地 `11434` 端口启动了 Ollama 服务。运行以下 Docker 命令行，在本地启动 mAI：
...
关于在 mAI 中使用 Ollama 的更多信息，请查阅 [Ollama 使用](/zh/docs/usage/providers/ollama)。
