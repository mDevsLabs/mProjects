---
title: 在 Sealos 上部署 mAI 数据库版
description: 学习如何在 Sealos 上部署 mAI，包括准备 OpenAI API Key、点击部署按钮、绑定自定义域名等操作。
tags:
  - Sealos
  - mAI
  - OpenAI API Key
...
# 使用 Sealos 部署 mAI 数据库版

<Callout type="info">
  本文假设你已经熟悉 mAI
  服务器数据库版的部署基本原理和流程，因此只包含与核心环境变量配置相关的内容。如果你对 mAI
  服务器数据库版的部署原理不熟悉，请先参考[快速开始](/zh/docs/self-hosting/start)。
</Callout>

在 Sealos 的 mAI 数据库版应用中总共包含有以下四个服务：
...
这里是在 Sealos 上部署 mAI 服务器数据库版的流程：
...
<Steps>
  ...
  在弹窗中填写应用的名称为 `mAI`，然后点击 `Create application` 按钮。接下来啥也不用填，直接点击底部的 `Finish and done` 按钮就创建完成了。
  ...
  在 `mAI` 应用中找到以下三个参数，后面部署 mAI 数据库版时需要用到。
...
![mAI 数据库版部署完成页面](https://raw.githubusercontent.com/labring-actions/templates/main/template/lobe-chat-db/images/lobe-chat-db-deployment-done.png)

找到公网地址，复制下来，后面需要用到。

![mAI 数据库版公网地址](https://raw.githubusercontent.com/labring-actions/templates/main/template/lobe-chat-db/images/lobe-chat-db-public-address.png)

## 部署后配置

进入 Logto 的 `Applications` 页面，找到 `mAI` 应用，点击进入应用详情页面。
...
其中 `https://<lobehub-db-public-address>` 为 mAI 数据库版的公网地址。

填完之后点击 `Save changes` 按钮保存配置。

现在通过 `https://<lobehub-db-public-address>` 访问 mAI 数据库版，点击左上角的头像，然后点击【登录 / 注册】按钮：

![mAI 数据库版登录页面](https://raw.githubusercontent.com/labring-actions/templates/main/template/lobe-chat-db/images/lobe-chat-db-login.png)

接下来会跳转到 Logto 的登录页面，点击【注册】注册一个账号。

![mAI 数据库版注册页面](https://raw.githubusercontent.com/labring-actions/templates/main/template/lobe-chat-db/images/lobe-chat-db-register.png)

注册完成后，即可使用 Logto 登录 mAI 数据库版。

![mAI 数据库版登录成功页面](https://raw.githubusercontent.com/labring-actions/templates/main/template/lobe-chat-db/images/lobe-chat-db-login-success.png)
