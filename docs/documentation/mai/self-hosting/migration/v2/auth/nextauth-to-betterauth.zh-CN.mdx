...
description: 将 mAI 部署从 NextAuth 身份验证迁移到 Better Auth 的指南，包括简单迁移和完整迁移选项。
tags:
  - 身份验证服务
  - Better Auth
  - NextAuth
...
本指南帮助您将现有的基于 NextAuth 的 mAI 部署迁移到 Better Auth。

<Callout type={'info'}>
  Better Auth 是 mAI 推荐的身份验证解决方案。它提供更简单的配置、更多的 SSO 提供商支持，以及更好的自托管体验。
</Callout>
...
如果登录时跳转到 `signin?callbackUrl=...&error=email_not_found`，说明 mAI 无法获取用户邮箱。
...
配置 SSO 连接时（如 Auth0 中的 GitHub 连接），务必在 **Attributes** 部分勾选 **Email Address** 权限。mAI 需要用户邮箱进行身份认证。
...
<Callout type={'warning'}>
  **Casdoor 用户注意**：Casdoor 不要求用户必须配置邮箱，但 mAI 强依赖邮箱进行身份认证。如果因为大量用户没有邮箱而感觉迁移困难，建议暂时停留在 [v2.0.0-next.344](https://github.com/lobehub/lobehub/releases/tag/v2.0.0-next.344) 版本。后续官方计划提供用户端自助迁移功能，届时没有邮箱的用户登录时会被重定向到绑定邮箱页面。
</Callout>

解决方案：

1. 先在 mAI 中配置身份提供商的 Webhook 以同步用户数据：
...
3. 用户数据通过 Webhook 同步到 mAI 后即可正常登录
