...
title: Codex
description: >-
  Delegate OpenAI Codex inside mAI — chat with the Codex CLI from your
...
Codex is OpenAI's coding agent that edits files, runs commands, and ships changes from your terminal. In mAI, you can delegate Codex from the desktop app — keep the chat UX you already use, while Codex does the work locally with full access to your project.
...
## What Is Codex in mAI?

A bridge between mAI's chat UI and the Codex CLI running on your machine. mAI spawns the Codex CLI as a local subprocess, streams its events into a chat conversation, and renders Codex's tool output — file changes, todo lists, command runs — as first-class chat blocks. You drive the agent in natural language; Codex executes locally with your environment, credentials, and project context.

## Requirements

- **mAI desktop app** — Codex agents only work in the desktop build. The web app cannot spawn local processes.
- **Codex CLI installed** — the `codex` command must be available on your `PATH`.
- **Signed in** — you must run `codex` once in a terminal to authenticate before mAI can drive it.
...
If mAI can't find the CLI, it shows an **Install Codex CLI** prompt with the same commands and an **Open System Tools** button — click it after installing to re-detect the CLI.

## Add Codex in mAI

When mAI detects the Codex CLI on your machine, an **Add Codex** recommendation card appears on the home page tagged "Coding Agent". Click it to create a Codex agent in one step.
...
If you change folders and the saved Codex thread can't be resumed safely, mAI shows: *"The saved Codex thread could not be resumed safely, so a new conversation has started for this topic."*

## What Gets Rendered in Chat

mAI renders Codex's tool calls with purpose-built blocks instead of raw JSON:
...
In any of these cases, mAI starts a fresh conversation automatically.
...
- **Desktop only** — the Codex agent runs in the mAI desktop app. The web app cannot spawn the CLI.
- **One sign-in per machine** — Codex shares its authentication with the global CLI. If `codex` works in your terminal, it works in mAI.
- **Working-directory-bound** — sessions don't follow you across folders or machines.

## Tips

- **Run `codex` once in a terminal first** — sign-in happens at the CLI level, not in mAI.
