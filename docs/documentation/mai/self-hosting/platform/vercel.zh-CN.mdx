---
title: 在 Vercel 上部署 mAI
description: 本文详细介绍如何在 Vercel 中部署 mAI，包括数据库配置、身份验证服务配置和 S3 存储服务的设置步骤。
tags:
  - 服务端数据库
  - Postgres
  - Better Auth
...
本文将详细介绍如何在 Vercel 中部署 mAI，包括： 1）数据库配置；2）身份验证服务配置；3） S3 存储服务的设置步骤。
...
<Steps>
  ...
  该部分最后需要添加 `APP_URL` 环境变量，用于指定 mAI 应用的 URL 地址。
</Steps>
...
<Steps>
  ...
  你需要获取 S3 的访问密钥，以便 mAI 的服务端有权限访问 S3 存储服务。在 R2 中，你可以在账户详情中配置访问密钥：
...
通过上述步骤之后，我们应该就完成了服务端数据库的配置。接下来我们可以将 mAI 部署到 Vercel 上，然后访问你的 Vercel 链接，验证服务端数据库是否正常工作。
