---
title: 将 mAI 连接到 Lark
description: 了解如何创建 Lark 自定义应用并将其连接到您的 mAI 代理作为消息渠道，使您的 AI 助手能够在 Lark 聊天中与团队成员互动。
tags:
  - Lark
  - 消息渠道
  - 机器人设置
...
通过将 Lark 渠道连接到您的 mAI 代理，团队成员可以直接在 Lark 的私聊和群组对话中与 AI 助手互动。
...
<Steps>
  ...
  点击 **Create Enterprise App**。填写应用名称（例如 "mAI Assistant"）、描述和图标，然后提交表单。
...
<Steps>
  ...
  在 mAI 中，导航到您的代理设置，然后选择 **渠道** 标签。点击平台列表中的 **Lark**。
...
<Steps>
  ...
  将您从 mAI 复制的 **Event Subscription URL** 粘贴到 **Request URL** 字段中。平台会自动验证端点。
...
回到 mAI 的渠道设置，点击 **Test Connection** 以验证凭证。然后在 Lark 中搜索您的机器人名称并发送消息，确认其是否响应。
...
获取方式：先用任意消息私信 bot 一次，查看入站事件 payload 中发送方的 `open_id` 字段，那就是你的。Lark 开发者后台也提供 **User ID 查询** 工具，用手机号 / 邮箱反查 `open_id`。粘贴到 mAI 高级设置的 **你的平台用户 ID** 字段。
...
- **Event Subscription URL 验证失败（Webhook 模式）：** 确保您已在 mAI 中保存配置，并正确复制了 URL。
- **机器人未响应：** 验证应用已发布并获得批准，机器人功能已启用，并订阅了 `im.message.receive_v1` 事件。
- **机器人不回私信：** 在 mAI 的 **高级设置** 检查 **私信策略**。如果是 `Disabled`，改成 `Open` 或 `Allowlist`；如果是 `Allowlist`，确认发起方的 `open_id` 已加入 **允许的用户 ID**。
- **权限错误：** 确保所有所需权限已在开发者门户中添加并获得批准。
- **测试连接失败：** 仔细检查 App ID 和 App Secret。确保您在 mAI 的渠道设置中选择了 "Lark"（而不是 "飞书"）。
