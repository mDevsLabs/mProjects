---
title: Connect mAI to WeChat
description: >-
  Learn how to connect a WeChat bot to your mAI agent via the iLink Bot API,
...
# Connect mAI to WeChat

By connecting a WeChat channel to your mAI agent, users can interact with the AI assistant through WeChat private chats and group conversations.
...
In mAI, navigate to your agent's settings, then select the **Channels** tab. Click **WeChat** from the platform list.
...
<Steps>
  ...
  Open WeChat on your phone, go to **Scan** (via the + button in the top right), and scan the QR code displayed in mAI.
  ...
  Once confirmed, mAI will automatically save your credentials and connect the bot. You should see a success message in the channel settings.
</Steps>

## Step 3: Test the Bot

Open WeChat, find your bot contact, and send a message. The bot should respond through your mAI agent.
...
1. When you scan the QR code, mAI obtains a bot token from WeChat's iLink API
2. mAI continuously polls the iLink API for new messages (\~35 second intervals)
3. When a message arrives, it is routed through the mAI agent for processing
