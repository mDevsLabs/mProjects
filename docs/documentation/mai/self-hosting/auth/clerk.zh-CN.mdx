...
<Callout type={'warning'}>
  Clerk 已从 mAI 中完全移除。设置任何 `CLERK_*` 环境变量都会导致部署在构建 / 启动阶段失败。本页仅作历史参考保留，请改用 [Clerk 迁移至 Better Auth 指南](/zh/docs/self-hosting/migration/v2/auth/clerk-to-betterauth)。
</Callout>
