---
title: 使用 Docker 部署 mAI 数据库
description: 详细步骤教你如何在 Docker 中部署 mAI 服务端数据库。
tags:
  - Docker
  - mAI
  - 数据库部署
...
<Callout type="info">
  本文已经假定你了解了 mAI
  的部署基本原理和流程，因此只包含核心环境变量配置的内容。如果你还不了解 mAI
...
<Steps>
  ...
  <Callout type="info">
    ParadeDB 镜像包含了 pgvector（向量搜索）和 pg\_search（全文搜索）插件，是 mAI 实现 RAG 和知识库搜索的重要构件。
  </Callout>
...
mAI 也支持直接在本地的 Mac/Windows 本地使用。
