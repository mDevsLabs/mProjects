---
title: 在 mAI 中配置 Authelia 身份验证服务
description: 学习如何在 mAI 中配置 Authelia 身份验证服务，包括创建提供程序、配置环境变量和部署 mAI。详细步骤和必要环境变量设置。
tags:
  - Authelia
  - 身份验证
  - 单点登录
...
<Steps>
  ...
  我们现在默认您已经了解了如何使用 Authelia。假设您的 mAI 实例部署在 `https://lobe.example.com/` 中。注意，目前只有 `localhost` 支持 HTTP 访问，其他域名需要启用 TLS，否则 Authelia 默认将主动中断身份认证。
  ...
  请您确保 `secret` 和 `redirect_urls` 替换成您自己的值。注意！Authelia 中配置 `secret` 是密文，即加盐哈希值。其对应的明文稍后需要填写在 mAI 中。
...
<Callout type={'info'}>
  部署成功后，用户将可以使用 Authelia 中配置的用户通过身份认证并使用 mAI。
</Callout>
