---
title: Connect mAI to LINE
description: >-
  Learn how to connect a LINE Messaging API bot to your mAI agent,
...
By connecting a LINE channel to your mAI agent, users can interact with the AI assistant through LINE direct messages, group chats, and multi-person rooms. The integration uses the official **LINE Messaging API** — there is no third-party broker between LINE and mAI.

## Prerequisites

- A mAI account with an active subscription
...
<Steps>
  ...
  - Pick a **Provider** that will own this channel in the Developers Console — reuse an existing one, or create a fresh one (e.g. "mAI").
  ...
  Open the **Basic settings** tab and copy the **Channel secret** — mAI uses it to verify webhook signatures in Step 4.

  > **Note:** the **"Your user ID"** field on the same tab is **your own** LINE user ID, **not** the bot's destination user ID. Both have the identical `U` + 32 hex format, but mAI needs the bot's, which is resolved automatically from the Channel Access Token in Step 4.
...
By default the LINE Official Account Manager auto-replies to user messages and sends a greeting on first contact. These compete with mAI's responses, so they must be turned off.
...
## Step 4: Configure LINE in mAI

<Steps>
  ### Open Channel Settings

  In mAI, navigate to your agent's settings, then select the **Channels** tab. Click **LINE** from the platform list.

  ### Fill in the credentials

  Recommended order — paste the token first so mAI can auto-fill the Destination User ID:

  1. **Channel Access Token** — paste the long-lived token issued in Step 2.
  2. **Destination User ID** — click **Fetch from LINE** next to this field. mAI calls `GET /v2/bot/info` with the token you just pasted and fills in the bot's `userId` (33 chars, starts with `U`) for you. You can also type/paste it manually if you already have it.
  ...
  Click **Save Configuration**. mAI will encrypt your credentials, call `GET /v2/bot/info` once to verify the token works and that the bot user ID matches your Destination User ID, and surface a **Webhook URL** for the next step.

  > **Note:** Unlike Telegram, the LINE Messaging API does not allow programmatic webhook registration. mAI cannot wire the URL for you — you must paste it in the LINE Developers Console yourself in Step 5.
</Steps>
...
<Steps>
  ...
  In mAI's LINE channel detail page, copy the **Webhook URL** displayed under the credentials section. It looks like `https://app.lobehub.com/api/agent/webhooks/line/<your-destination-user-id>`.

  ### Paste it in the LINE Developers Console

  Back in the **Messaging API** tab of your channel:

  - **Webhook URL:** paste the mAI Webhook URL.
  - Click **Update**.
  - Click **Verify**. LINE sends a signed `POST` with `events: []` to mAI, which responds 200 if the Channel Secret matches.
...
<Steps>
  ...
  Send any message to the bot on LINE. Within a few seconds your mAI agent should reply.

  ### Run Test Connection (optional)

  Click **Test Connection** in mAI's channel settings to re-verify the token and the bot identity match. Errors are surfaced with the exact LINE error message.
</Steps>
...
| ------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Destination User ID**  | Yes      | The bot's user ID (`U` + 32 hex chars). mAI auto-fills this via the **Fetch from LINE** button (calls `GET /v2/bot/info` with the access token); the LINE Developers Console UI does not display the value. Used as the bot identity and webhook path segment. |
...
LINE's Messaging API has a few specifics that mAI maps as follows:

- **Markdown** — LINE renders text messages as **plain text** only. mAI strips Markdown markup before sending so emphasis / heading / list markers are removed.
- **Message editing** — the Messaging API does not support editing sent messages, so mAI only sends the **final reply**, not per-step progress edits.
...
- **Outbound** — mAI uses the **push API** (`/v2/bot/message/push`) rather than the reply API, because the reply token expires in \~60s while agent generation can take longer. Push messages count against your channel's monthly quota for paid plans; the free Developer Trial is unlimited.
...
- **"Verify" fails in the LINE Developers Console.** The Channel Secret in mAI must match the value shown on the LINE Developers Console **Basic settings** tab exactly. Re-paste it, save, and try again.
- **`Authentication failed.` on Save / Test Connection.** Your Channel Access Token is invalid or expired. Re-issue the long-lived token in the Messaging API tab and paste the new value into mAI.
...
- **Webhook delivery is rejected with `401 Invalid signature`.** The Channel Secret in mAI doesn't match the one shown in the LINE Developers Console. Update mAI with the correct Channel Secret.
