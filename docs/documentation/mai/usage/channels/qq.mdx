---
title: Connect mAI to QQ
description: >-
  Learn how to create a QQ bot and connect it to your mAI agent as a message
...
By connecting a QQ channel to your mAI agent, users can interact with the AI assistant through QQ group chats, guild channels, and direct messages.
...
mAI supports two connection modes for QQ bots:
...
<Steps>
  ...
  If you are using **Webhook mode**, navigate to **Development Settings** → **Callback Configuration** in the QQ Open Platform. You will need to paste the mAI Callback URL here after completing Step 2.

  If you are using **WebSocket mode** (default), skip this step — no callback URL is needed.
</Steps>

## Step 2: Configure QQ in mAI

<Steps>
  ### Open Channel Settings

  In mAI, navigate to your agent's settings, then select the **Channels** tab. Click **QQ** from the platform list.
...
<Steps>
  ...
  Go back to the QQ Open Platform, navigate to **Development Settings** → **Callback Configuration**. Paste the **Callback URL** you copied from mAI.
  ...
  The QQ Open Platform will send a verification request to your Callback URL. mAI handles this automatically using Ed25519 signature verification.
</Steps>
...
Click **Test Connection** in mAI's channel settings to verify the integration. Then open QQ, find your bot, and send a message. The bot should respond through your mAI agent.
...
To get it: DM the bot once with any message and check the server logs for the `user_openid` field on the inbound event payload (or read it from the OpenAPI dashboard if available). Paste that ID into **Your Platform User ID** in mAI's Advanced Settings.
...
- **Callback URL verification failed (Webhook mode):** Ensure you saved the configuration in mAI first and the URL was copied correctly. mAI handles Ed25519 verification automatically.
...
- **Test Connection failed:** Double-check the App ID and App Secret in mAI's channel settings.
