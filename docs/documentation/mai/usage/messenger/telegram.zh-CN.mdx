---
title: 在 Telegram 使用 mAI
description: >-
  将 mAI 账号关联到 Telegram 上的官方 mAI 机器人。给机器人发送
...
Telegram 是 Messenger 里最简单的平台：只有一个全局 mAI 机器人，任何 Telegram 账号都能直接私聊它。你只需要把自己的 mAI 账号绑一次。
...
在 mAI 中打开 **设置 → Messenger**，点击 **Telegram** 卡片。
...
在弹窗里点 **在 Telegram 中打开**，或用手机扫描二维码，Telegram 会自动打开官方 mAI 机器人。
...
点击机器人回复里的链接，会跳回 mAI 网页。如未登录请先登录，然后选择一个 **默认 Agent** —— 之后你在该机器人里发的每条消息都会路由到这个 Agent。
...
- **在 mAI 里** —— 打开 **设置 → Messenger → Telegram**，在你的关联行里使用 Agent 选择器。
...
> 在 mAI 里断开不会从 Telegram 聊天列表里删掉机器人 —— 如果不想再看到它，可以在 Telegram 里手动删除该会话。
...
- **"This account is already linked"** —— 这个 Telegram 账号已绑定到另一个 mAI 账号。请用那个 mAI 账号登录管理，或先在那边解绑。
- **"Another Telegram account is already linked"** —— 你的 mAI 账号在 Telegram 上已有关联。先在 **设置 → Messenger → Telegram** 断开旧关联，再绑定新的 Telegram 账号。
