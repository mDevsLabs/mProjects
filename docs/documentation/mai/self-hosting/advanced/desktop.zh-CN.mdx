---
title: 与 mAI 桌面端自动同步
description: 配置 mAI 桌面端，获取端同步体验
tags:
  - mAI
  - 桌面端
  - 自托管
...
mAI 桌面端为用户提供了更好的使用体验，同时也扩展了 mAI 的能力边界，允许用户在离线环境下使用 mAI。本文档将指导您如何配置 mAI 桌面端以及如何将其与自托管实例连接。

## 系统要求

mAI 桌面端使用 Electron 构建，支持以下操作系统：
...
- **Beta 版本**：所有合并到 main 分支并自动发布版本，都会自动创建为 mAI 桌面端的 Beta 版本
...
mAI 桌面端可以与您自托管的 mAI 实例连接，以便您可以在桌面端使用您的自托管配置。
...
如果您使用一键部署方式（如 Vercel、Railway 等平台）部署 mAI，您需要：
...
1. 打开 mAI 桌面端
2. 点击右上角 wifi 图标
3. 输入您的自托管实例地址（例如：`https://your-mAI-instance.com`）
4. 点击 "连接" 按钮
...
- 文件名示例：
  - Apple Silicon: `mAI-Beta-1.83.6-arm64.dmg` 或 `mAI-Beta-1.83.6-arm64-mac.zip`
  - Intel: `mAI-Beta-1.83.6-x64.dmg` 或 `mAI-Beta-1.83.6-mac.zip`
