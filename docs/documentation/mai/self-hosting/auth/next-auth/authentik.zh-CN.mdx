---
title: 在 mAI 中配置 Authentik 身份验证服务
description: 学习如何在 mAI 中配置 Authentik 身份验证服务，包括创建提供程序、配置环境变量和部署 mAI。详细步骤和必要环境变量设置。
tags:
  - Authentik
  - 身份验证
  - 单点登录
...
<Callout type={'info'}>
  部署成功后，用户将可以使用 Authentik 中配置的用户通过身份认证并使用 mAI。
</Callout>
