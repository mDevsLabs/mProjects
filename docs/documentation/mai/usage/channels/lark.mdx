---
title: Connect mAI to Lark
description: >-
  Learn how to create a Lark custom app and connect it to your mAI agent as
...
By connecting a Lark channel to your mAI agent, team members can interact with the AI assistant directly on Lark through private chats and group conversations.
...
mAI supports two connection modes for Lark bots:
...
<Steps>
  ...
  Click **Create Enterprise App**. Fill in the app name (e.g., "mAI Assistant"), description, and icon, then submit the form.
...
## Step 3: Configure Lark in mAI

<Steps>
  ### Open Channel Settings

  In mAI, navigate to your agent's settings, then select the **Channels** tab. Click **Lark** from the platform list.
...
<Steps>
  ...
  Paste the **Event Subscription URL** you copied from mAI into the **Request URL** field. The platform will verify the endpoint automatically.
  ...
  This allows your app to receive messages and forward them to mAI.
  ...
  Go back to mAI's channel settings and fill in:
...
Back in mAI's channel settings, click **Test Connection** to verify the credentials. Then find your bot on Lark by searching its name and send it a message to confirm it responds.
...
To get it: DM the bot once and inspect the inbound event payload — the `open_id` field on the sender is yours. The Lark Developer Portal also exposes a **User ID** lookup that maps mobile/email to `open_id`. Paste it into **Your Platform User ID** in mAI's Advanced Settings.
...
- **Event Subscription URL verification failed (Webhook mode):** Ensure you saved the configuration in mAI first, and the URL was copied correctly.
...
- **Bot ignores DMs:** Open **Advanced Settings** in mAI and check **DM Policy**. If it is `Disabled`, switch to `Open` or `Allowlist`. If it is `Allowlist`, confirm the sender's `open_id` is listed in **Allowed User IDs**.
- **Permission errors:** Confirm all required permissions are added and approved in the Developer Portal.
- **Test Connection failed:** Double-check the App ID and App Secret. Make sure you selected "Lark" (not "飞书") in mAI's channel settings.
