...
description: 详细指南：如何配置 Upstash Redis 用于 mAI 的缓存和会话存储。
tags:
  - Upstash
  - Redis
  - 缓存
...
[Upstash](https://upstash.com/) 是一个 Serverless Redis 服务，提供免费额度和按量付费模式，非常适合 mAI 部署使用。
...
<Steps>
  ...
  <Callout type={'info'}>
    Upstash 使用 `rediss://`（双's'）表示 TLS 连接，mAI 自动支持此格式。
  </Callout>
</Steps>
