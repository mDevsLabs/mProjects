---
title: Customizing Provider Model List in mAI for Deployment
description: >-
  Learn how to customize the model list in mAI for deployment with the
...
mAI supports customizing the model list during deployment. This configuration is done in the environment for each [model provider](/docs/self-hosting/environment-variables/model-provider).
...
- Confirm the model actually supports the capability in the provider's API (mAI cannot enable capabilities the API doesn't provide)
- Verify you are running a recent enough version of mAI
