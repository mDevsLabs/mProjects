...
description: Learn how to implement a new authentication provider using Better Auth in mAI.
tags:
  - Authentication
  - Better Auth
  - mAI
...
mAI uses [Better Auth](https://www.better-auth.com/) as the external authentication service. Better Auth is an open-source authentication library that provides a simple way to implement authentication and authorization features. This document will introduce how to use Better Auth to implement a new authentication provider.

## Architecture Overview

To add a new authentication provider in mAI (for example, adding GitLab), you need to follow the steps below:
...
Now, you can use GitLab as your provider to implement the authentication feature in mAI.
