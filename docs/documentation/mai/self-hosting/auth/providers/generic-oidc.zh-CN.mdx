---
title: 在 mAI 中配置 Generic OIDC 身份验证
description: 学习如何使用 Generic OIDC 集成将 mAI 与任何 OpenID Connect 提供商对接。
tags:
  - Generic OIDC
  - OpenID Connect
  - 身份验证
...
使用 Generic OIDC 可以将 mAI 与任何符合 OpenID Connect 标准的身份提供商集成，适用于不在我们支持列表中的提供商。
...
<Callout type={'info'}>
  部署成功后，用户将可以通过你的 OIDC 提供商身份认证并使用 mAI。
</Callout>
