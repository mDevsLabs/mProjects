---
title: 将 mAI 连接到 Telegram
description: >-
  学习如何创建一个 Telegram 机器人并将其连接到 mAI 代理作为消息渠道，使您的 AI 助手能够在 Telegram
  私聊和群组对话中与用户互动。
tags:
  - Telegram
  - 消息渠道
...
通过将 Telegram 渠道连接到您的 mAI 代理，用户可以通过 Telegram 私聊和群组对话与 AI 助手互动。
...
## 第二步：在 mAI 中配置 Telegram

<Steps>
  ### 打开渠道设置

  在 mAI 中，导航到您的代理设置，然后选择 **渠道** 标签页。从平台列表中点击 **Telegram**。
  ...
  点击 **保存配置**。mAI 将自动向 Telegram 注册 Webhook URL，无需手动复制 URL。
...
在 mAI 的渠道设置中点击 **测试连接** 以验证集成。然后打开 Telegram，搜索您的机器人用户名并发送消息。机器人应通过您的 mAI 代理进行响应。
...
获取方式：打开 Telegram，私信 [@userinfobot](https://t.me/userinfobot)，它会把你的数字用户 ID 回给你。粘贴到 mAI 高级设置的 **你的平台用户 ID** 字段。
...
- **Webhook 注册失败：** 确保您的 mAI 订阅处于活动状态。Telegram 要求 Webhook 使用 HTTPS 端点，mAI 会自动提供。
