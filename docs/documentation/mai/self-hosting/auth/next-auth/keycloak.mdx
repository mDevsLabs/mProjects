---
title: Configuring Keycloak Authentication Service in mAI
description: >-
  Learn how to configure the Keycloak authentication service in mAI,
...
# Configuring Keycloak Authentication Service in mAI

<Callout type={'warning'}>
  NextAuth (Auth.js) has been fully removed from mAI. The `NEXT_AUTH_SSO_PROVIDERS` and `AUTH_URL` variables shown below will now cause the deployment to fail at build/startup. This page is kept for historical reference only — use the [Better Auth provider guides](/docs/self-hosting/auth) and the [NextAuth to Better Auth migration guide](/docs/self-hosting/migration/v2/auth/nextauth-to-betterauth) instead.
</Callout>
...
<Callout type={'tip'}>
  If you want to privately deploy Keycloak, we recommend using it together with mAI via Docker
...
<Steps>
  ...
  1. Create a new Realm
     ...
     - Enter a name, such as "mAI", then click "Create"
  ...
  2. Create a Client
     ...
     - On the "Login settings" page:
       - Valid redirect URIs:
         ...
         - Local network IP deployment: `http://mAI_IP:3210/api/auth/callback/keycloak`
         - Public environment: `https://lobe.example.com/api/auth/callback/keycloak`
       - Web origins: Add your mAI domain or IP
       - Click "Save"
  ...
  Set the obtained client ID and client secret as `AUTH_KEYCLOAK_ID` and `AUTH_KEYCLOAK_SECRET` in the mAI environment variables.

  Configure the mAI environment variable `AUTH_KEYCLOAK_ISSUER` as:

  - `http://localhost:8080/realms/mAI` for local development environment
  - `http://KEYCLOAK_IP:8080/realms/mAI` for privately deployed Keycloak on a local network
  - `https://lobe-auth-api.example.com/realms/mAI` for Keycloak deployed in a public environment
  ...
  | `NEXT_AUTH_SSO_PROVIDERS` | Required | Select the single sign-on provider for mAI. For Keycloak, fill in `keycloak`.                                                                                                        |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate through Keycloak and use mAI.
</Callout>
