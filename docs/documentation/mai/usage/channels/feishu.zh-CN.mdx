---
title: 将 mAI 连接到飞书
description: 了解如何创建飞书自定义应用并将其连接到您的 mAI 代理作为消息渠道，使您的 AI 助手能够在飞书聊天中与团队成员互动。
tags:
  - 飞书
  - 消息渠道
  - 机器人设置
...
通过将飞书渠道连接到您的 mAI 代理，团队成员可以直接在飞书的私聊和群组对话中与 AI 助手互动。
...
<Steps>
  ...
  点击 **创建企业应用**。填写应用名称（例如 "mAI 助手"）、描述和图标，然后提交表单。
...
<Steps>
  ...
  在 mAI 中，导航到您的代理设置，然后选择 **渠道** 标签。点击平台列表中的 **飞书**。
...
<Steps>
  ...
  将您从 mAI 复制的 **事件订阅 URL** 粘贴到 **请求 URL** 字段中。平台会自动验证端点。
...
回到 mAI 的渠道设置，点击 **测试连接** 以验证凭证。然后在飞书中搜索您的机器人名称并发送消息，确认其是否响应。
...
获取方式：先用任意消息私信 bot 一次，查看入站事件 payload 中发送方的 `open_id` 字段，那就是你的。飞书开发者后台也提供 **User ID 查询** 工具，用手机号 / 邮箱反查 `open_id`。粘贴到 mAI 高级设置的 **你的平台用户 ID** 字段。
...
- **事件订阅 URL 验证失败（Webhook 模式）：** 确保您已在 mAI 中保存配置，并正确复制了 URL。
